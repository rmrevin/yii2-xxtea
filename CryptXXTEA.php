...
class CryptXXTEA
{
    ...
    {
        if (!is_string($str)) {
            throw new InvalidParamException(get_class($this).'::encrypt() - The plain text must be a string.');
        }
        if ($str == '') {
            return '';
        }
        ...
        while (0 < $q--) {
            ...
            for ($p = 0; $p < $n; $p++) {
                ...
                $mx = $this->_int32(
                    ...
                    ) ^ $this->_int32(($sum ^ $y) + ($this->_key[$p & 3 ^ $e] ^ $z));
    ...
    {
        if (!is_string($str)) {
            throw new InvalidParamException(get_class($this).'::decrypt() - The cipher text must be a string.');
        }
        if ($str == '') {
            return '';
        }
    ...
    {
        if (!is_string($key)) {
            throw new InvalidParamException(get_class($this) . '::setKey() - The secret key must be a string.');
        }
        $k = $this->_str2long($key, false);
        if (count($k) > 4) {
            throw new InvalidParamException(get_class(
                $this
            ) . '::setKey() - The secret key cannot be more than 16 characters.');
        } elseif (count($k) == 0) {
            throw new InvalidParamException(get_class($this) . '::setKey() - The secret key cannot be empty.');
        } elseif (count($k) < 4) {
            for ($i = count($k); $i < 4; $i++) {
                $k[$i] = 0;
            }
